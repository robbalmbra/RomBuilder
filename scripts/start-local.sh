...
if [ "$scp_check" -eq 5 ]; then
  ...
  if [ ! -f "$HOME/.ssh/id_rsa" ]; then
    ...
    echo "ssh -i "$HOME/.ssh/id_rsa" -o StrictHostKeyChecking=no -o BatchMode=yes -o ConnectTimeout=5 ${SCP_USERNAME}@${SCP_HOST}"
    ssh -i "$HOME/.ssh/id_rsa" -o StrictHostKeyChecking=no -o BatchMode=yes -o ConnectTimeout=5 "${SCP_USERNAME}@${SCP_HOST}" > /dev/null 2>&1
    ret=$?
    if [ $ret -gt 1 ]; then
